...
template GetHighestScore (players_num) {
    signal input scores[players_num];
    
    signal input scores_val_secret[players_num][2]; //associating each score to a 2d array contains the secret and the score
    
    signal output highest_score[2];
    signal output scores_validity[players_num];
    
    component hashes[players_num];
